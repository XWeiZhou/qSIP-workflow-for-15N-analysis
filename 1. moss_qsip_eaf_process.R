...
inital.qsip <- read.csv("qSIP_all_data sheet.csv")[,c(-1,-3)] 
...
tx <- unique(moss_qsip[, c('asv_id', 'Kingdom', 'Phylum', 'Class', 'Order', 'Family', 'Genus')])
unassign <- tx[Kingdom == 'Unassigned', asv_id]
euk <- tx[Kingdom == 'Eukaryota', asv_id]
arch <- tx[Kingdom == 'Archaea', asv_id]
mito_chloro <- tx[, tax_string := paste(Phylum, Class, Order, Family, Genus, sep = ';')][grepl('mitochond|chloroplast', tax_string, ignore.case = T), asv_id]
...
cat('Unassigned\n')
seq_summary(moss_qsip[asv_id %in% unassign, ..ssc])
cat('Eukarya\n')
seq_summary(moss_qsip[asv_id %in% euk, ..ssc])
cat('Archaea\n')
seq_summary(moss_qsip[asv_id %in% arch, ..ssc])
cat('Mitochondria, Chloroplasts\n')
seq_summary(moss_qsip[asv_id %in% mito_chloro, ..ssc])

# Filter out lineages
moss_qsip <- moss_qsip[!asv_id %in% c(arch, mito_chloro, euk, unassign)]
...
moss_qsip_after3fraction <- freq_filter(moss_qsip,  min_freq = 3, filter_target = 'sampleID', tax_id = 'asv_id')
...
unique_asv_ids <- lapply(split(moss_qsip_after3fraction, moss_qsip_after3fraction$sampleID), function(x) unique(x$asv_id))

df_unique_asv_ids <- 
  do.call(rbind, lapply(seq_along(unique_asv_ids), function(i) {
  data.frame(sampleID = names(unique_asv_ids)[i], asv_id = unique_asv_ids[[i]])
}))

# Count the number of OTUs per sampleID 
unique_sum <-df_unique_asv_ids %>% group_by(sampleID) %>% summarise(n =length(asv_id))
unique_sum
...
df_unique_asv_ids_rep <- merge(df_unique_asv_ids, group_sampleID_with_control_and_rep, by="sampleID", all.x = T, sort = F)
# View(df_unique_asv_ids_rep)

# Count occurrences of each OTU ID within moss  ecosystems under control and warming treatments
asv_id_counts <- with(df_unique_asv_ids_rep, table(Treatment, category, iso_trt,  asv_id))

# Convert the result to a data frame and rename columns
df_asv_id_counts <- as.data.frame(asv_id_counts)
names(df_asv_id_counts) <- c("Treatment", "ecosystem", "iso_trt","asv_id", "count")
df_asv_id_counts$count <- as.numeric(df_asv_id_counts$count)
...
num_rep = 2
moss_qsip_filtered_final <- df_asv_id_counts %>% filter(count >= num_rep)
...
unique_sum_2_rep <-moss_qsip_filtered_final %>% group_by(Treatment, ecosystem ,iso_trt) %>% summarise(n =length(asv_id))
unique_sum_2_rep
...
moss_control <- moss_qsip_filtered_final %>% filter(Treatment=="Control" & ecosystem=="moss") %>% group_by(asv_id) %>% filter(n() > 1) %>% distinct(asv_id)
moss_warming <- moss_qsip_filtered_final %>% filter(Treatment=="Warming" & ecosystem=="moss") %>% group_by(asv_id) %>% filter(n() > 1) %>% distinct(asv_id)

# Extract the corresponding data from the three-fraction dataset 
# based on the confirmed OTU names (asv_id)
moss_control_sip <- moss_qsip_after3fraction %>% filter(treatment =="Control"& ecosystem=="moss" & asv_id %in% moss_control$asv_id) 
moss_warming_sip <- moss_qsip_after3fraction %>% filter(treatment =="Warming"& ecosystem=="moss" & asv_id %in% moss_warming$asv_id) 
...
# moss_qsip[asv_id == "OTU_893" & sampleID == "W1_Moss_6"]
#convert the format
...
keep_cols <- setdiff(names(moss_qsip), c('asv_id', 'sampleID', 'fraction', 'Density.g.ml','avg_16S_g_soil', 'rel_abund', 'seq_abund'))
...
# creat an index tag with asv_id + treatment + ecosystem
data_eaf_taxa[, index_column := paste(asv_id,  treatment, sep = "_")]
data_eaf_pro[, index_column := paste(asv_id, treatment, sep = "_")]

#rename the data_eaf_pro
names(data_eaf_pro) <- c("asv_id","treatment","q2.5","q50","q97.5","p_val","index_column")
...
names(filtered_data) <-  c("asv_id", "sampleID", "timepoint", "isotope", "ecosystem", 
                  "treatment", "rep", "Kingdom", "Phylum", "Class", "Order", 
                  "Family", "Genus", "wvd", "abund", "light", "label", 
                  "gc_prop", "mw_light", "mw_label", "mw_max", "nat_abund", "eaf",
                  "asv_id.y",	"treatment.y",	"q2.5",	"q50",	"q97.5",	"p_val")
# View(filtered_data)

# write.csv(filtered_data,"filtered_data_taxa_pro_signi_minus_0_20250330_1000.csv")
write.xlsx(filtered_data, file = "filtered_data_taxa_pro_signi_minus_0_20250330_1000.xlsx", rowNames = FALSE)

## Extract sampleID and eaf for creating wide_OTU_moss_result_after_eaf_over_0.csv
select_OTU_venn <- filtered_data[,c("asv_id","sampleID","eaf")]
